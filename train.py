...
def get_defaultconfig():
    ...
    config = {'rnn_type'    : 'LeakyRNN',   # LeakyRNN, LeakyGRU, EILeakyGRU, GRU, LSTM
              'loss_type'   : 'lsq',        # type of loss function
              'activation'  : 'relu',       # relu, softplus, tanh, elu
              'tau'         : 100,          # ms
              'dt'          : 20,           # discretization time step
              'alpha'       : 0.2,
              'sigma_rec'   : 0.05,
              'sigma_x'     : 0.01,
              #'w_rec_init'  : 'diag',       # diag, randortho, randgauss, only used for leaky_rec
              #'l1_h'        : 1.0*0.0001,   # a default weak regularization prevents instability
              #'l2_h'        : 1.0*0,
              #'l2_weight'   : 0.0001*0,     #maddy changed: commented
              'l2_weight_init' : 0.0001*0,
              'target_perf' : None,         # 0.9, None
              'seed'        : 0,
              'rng'         : None,
...
def train(save_name,
    ...
    Args:
        ...
        rule_trains : list of lists of trained rules.
         rule_trains = [list1, list2, list3, ...]. Lists are trained sequentially.
         Within each list, rules are trained simultaneously

        rule_tests : list of rules to compute performance
        rule_prob_map : dictionary of relative probabilities of rules appearing during training

        run_analysis : list of strings deciding the analysis to be ran at the end
        
        w_rec_init   : diag, randortho, randgauss, only used for leaky_rec
        l1_h         : a default weak regularization on activity prevents instability
    ...
    if rule_trains is None:
        rule_trains = [rules_dict[ruleset]] # this will automatically train all rules
    ...
    for rule_train in rule_trains:
        if not hasattr(rule_train, '__iter__'):
            ...
            # Set default as 1.
            rule_prob = np.array([rule_prob_map.get(r, 1.) for r in rule_train])
    ...
    with tf.Session() as sess:
        ...
        if config['l2_weight_init'] > 0:
            ...
            for v, v_val in zip(model.var_list, anchor_vars):
                model.cost_reg += config['l2_weight_init']*tf.reduce_sum(tf.square(v-v_val))
        ...
        for i_rule_train, rule_train in enumerate(rule_trains):
            step = 1
            reached_target_perf = 0

            # At the beginning of new tasks
            if intsyn: # Only if using intelligent synapses
                ...
                if i_rule_train == 0:
                    ...
                    for v, w, v_val in zip(model.var_list, Omega0, v_current):
                        extra_cost += c_intsyn*tf.reduce_sum(tf.multiply(w, tf.square(v-v_val)))
            ...
            while step * batch_size_train <= rule_train_iters[i_rule_train]:
                try:
                    ...
                    if not hasattr(rule_train, '__iter__'):
                        ...
                        rule_train_now = config['rng'].choice(rule_train,
                                                              p=rule_probs[i_rule_train])

                    # Generate a random batch of trials. Each batch has the same trial length
                    trial = generate_trials(rule_train_now, config, 'random', batch_size=batch_size_train)

                    # Generating feed_dict. This is probably not efficient for GPUs
                    ...
                    if not intsyn:
                        ...
                        _, grads_and_vars_ = sess.run([model.optimizer, model.grads_and_vars],
                                                   feed_dict=feed_dict)
                    ...
                    if step % display_step == 0:
                        ...
                        for rule_test in rule_tests:
                            ...
                            for i_rep in range(n_rep):
                                ...
                                feed_dict = {model.x: trial.x,
                                             model.y: trial.y.reshape((-1,n_output)),
                                             model.c_mask: trial.c_mask}
                                c_lsq, c_reg = sess.run([model.cost_lsq, model.cost_reg], feed_dict=feed_dict)
                            ...
                            print('{:15s}'.format(rule_test) +
                                  '| cost {:0.6f}'.format(cost_tests[rule_test][-1])  +
                                  '| c_reg {:0.6f}'.format(creg_tests[rule_test][-1])  +
                        ...
                        log['trials']     = trials
                        log['times']      = times
                        log['rule_now']   = rule_now
                        log['cost_tests'] = cost_tests
                        log['perf_tests'] = perf_tests
                        log['cost_conts']  = cost_conts

                        tools.save_log(log, save_name)

                        if config['target_perf'] is not None:
                            if hasattr(rule_train, '__iter__'):
                                rule_tmp = rule_train
                            else:
                                rule_tmp = [rule_train]
                            perf_tests_mean = np.mean([perf_tests[r][-1] for r in rule_tmp])
                            if perf_tests_mean > config['target_perf']:
                                reached_target_perf += 1
                            else:
                                reached_target_perf = 0

                            if reached_target_perf >= 1:
                                # Has to pass the threshold for two consecutive tests
                                print('Performance reached the target: {:0.2f}'.format(target_perf))
...
#maddy added - function to create save_name
def to_savename(n_hidden = 64,
          seed          = None,
          activation    = 'tanh',
          rnn_type      = 'LeakyGRU',          
          w_rec_init    = 'diag', 
    ...
    if (max(l1_weight, l2_weight) > 0 and max(l1_h, l2_h) > 0 ):
        save_name = ('hidden_' + str(n_hidden) + '_seed_' + str(seed) + '_' + activation + '_' 
        ...
        save_name = ('hidden' + str(n_hidden) + '_seed_' + str(seed) + '_' + activation + '_' 
                     + rnn_type + '_' + w_rec_init + '_' + 
                     '_regu_None') #no regularization.     
    return save_name
...
if __name__ == '__main__':
    ...
    train('debug', n_hidden=64, seed=2, activation='tanh', rnn_type='LeakyGRU', run_analysis=run_analysis) #rob orig. 
...
#     
#     train(save_name = save_name, n_hidden=n_hidden, seed=seed, activation=activation, rnn_type=rnn_type, 
